...
def compute_query_window_similarity(model, inputs, window_size):
...
class QueryWindowSimilarityRunWriter:
    def __init__(self, root_dir, model_name, out_file, window_size, max_prefill_tokens):
...
class QueryWindowSimilaritySampleWriter:
    ...
    def capture_prefill(self, model, tokenizer, prompt_text, inputs, label, max_prefill_tokens=None):
        ...
        try:
            ...
            similarity_file_name = f"prefill_{prefill_index:03d}_query_window_similarity.npz"
            heatmap_file_name = f"prefill_{prefill_index:03d}_query_window_similarity.png"
            ...
            try:
                plot_query_window_similarity_heatmap(
                    ...
                    title=f"{label}: last {actual_window_size} token hidden-state cosine",
                )
