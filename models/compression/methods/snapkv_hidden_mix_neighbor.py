...
class SnapKVHiddenMix:
    ...
    def __init__(
        ...
        hidden_mix_neighbor_layers=2,
        hidden_mix_neighbor_n=None,
        ...
        **kwargs,
        ...
        if hidden_mix_neighbor_n is not None:
            hidden_mix_neighbor_layers = hidden_mix_neighbor_n
        self.budget = budget
        ...
        self.hidden_mix_neighbor_layers = self._validate_neighbor_layers(hidden_mix_neighbor_layers)
    ...
    def _validate_neighbor_layers(self, neighbor_layers):
        try:
            neighbor_layers = int(neighbor_layers)
        except (TypeError, ValueError) as exc:
            raise ValueError("hidden_mix_neighbor_layers must be a non-negative integer.") from exc
        if neighbor_layers < 0:
            raise ValueError("hidden_mix_neighbor_layers must be a non-negative integer.")
    ...
    def _mix_for_layer(self, group_layers, target_layer):
        ...
        if mix is None:
            return {"sources": (target_layer,), "weights": (1.0,)}
        return self._neighbor_limited_mix(mix, target_layer)

    def _neighbor_limited_mix(self, mix, target_layer):
        target_layer = int(target_layer)
        neighbor_layers = int(self.hidden_mix_neighbor_layers)
        filtered = [
            (int(source_layer), float(weight))
            for source_layer, weight in zip(mix["sources"], mix["weights"])
            if abs(int(source_layer) - target_layer) <= neighbor_layers
        ]
        if not filtered:
            return {"sources": (target_layer,), "weights": (1.0,)}

        weight_sum = float(sum(weight for _, weight in filtered))
        if (not math.isfinite(weight_sum)) or weight_sum <= 0.0:
            return {"sources": (target_layer,), "weights": (1.0,)}

        return {
            "sources": tuple(source_layer for source_layer, _ in filtered),
            "weights": tuple(weight / weight_sum for _, weight in filtered),
        }
