...
class SnapKVHiddenMix:
    ...
    def _parse_hidden_mix_profile(self, raw_profile, profile_path):
        ...
        for group_idx, group_spec in enumerate(groups):
            ...
            for target_text, mix_spec in raw_mix.items():
                ...
                if "attn_output_ratio_sum" in mix_spec:
                    layer_budget_scores[target_layer] = self._parse_layer_budget_score(
                        mix_spec["attn_output_ratio_sum"],
                        group_idx,
                        target_layer,
                    )

            layer_budget_scores.update(
                self._parse_group_layer_budget_scores(group_spec, layers, group_idx)
            )
            group_profiles[layers] = {"mix": mix}

        has_layer_budget_scores = all(layer in layer_budget_scores for layer in profiled_layers)
        layer_budget_score_total = (
            float(sum(layer_budget_scores[layer] for layer in profiled_layers))
            if has_layer_budget_scores
            else 0.0
        )
        use_layer_budget_scores = has_layer_budget_scores and layer_budget_score_total > 0.0

        return {
            "metric": raw_profile.get("metric"),
            "groups": group_profiles,
            "layer_to_group": layer_to_group,
            "profiled_layers": tuple(profiled_layers),
            "profiled_layer_count": len(profiled_layers),
            "layer_budget_scores": layer_budget_scores,
            "layer_budget_score_total": layer_budget_score_total,
            "use_layer_budget_scores": use_layer_budget_scores,
        }

    def _parse_group_layer_budget_scores(self, group_spec, layers, group_idx):
        raw_scores = None
        for key in (
            "attn_output_ratio_sums",
            "layer_attn_output_ratio_sums",
            "attn_output_ratio_sum",
        ):
            if key in group_spec:
                raw_scores = group_spec[key]
                break
        ...
        if isinstance(raw_scores, dict):
            parsed = {}
            for layer_text, raw_score in raw_scores.items():
                layer = int(layer_text)
                if layer not in layers:
                    raise ValueError(f"Profile group {group_idx} has attn_output_ratio_sum outside layers: {layer}.")
                parsed[layer] = self._parse_layer_budget_score(raw_score, group_idx, layer)
            return parsed

        if isinstance(raw_scores, list):
            if len(raw_scores) != len(layers):
                raise ValueError(f"Profile group {group_idx} attn_output_ratio_sum list must match layers length.")
            return {
                int(layer): self._parse_layer_budget_score(raw_score, group_idx, int(layer))
                for layer, raw_score in zip(layers, raw_scores)
            }

        if len(layers) != 1:
            raise ValueError(f"Profile group {group_idx} scalar attn_output_ratio_sum requires a single layer.")
        layer = int(layers[0])
        return {layer: self._parse_layer_budget_score(raw_scores, group_idx, layer)}

    def _parse_layer_budget_score(self, raw_score, group_idx, layer):
        score = float(raw_score)
        if (not math.isfinite(score)) or score < 0.0:
            raise ValueError(
                f"Profile group {group_idx} layer {layer} attn_output_ratio_sum must be finite and non-negative."
            )
        return score
    ...
    def update_kv_cache(
        ...
        layer_budget = self._layer_cache_budget()
        if self._should_append_to_cache(query_states, layer_cache):
            key_states, value_states = past_key_values.update(key_states, value_states, self.layer_idx)
        else:
            if self.model_config.update_kv is not True or key_states.shape[-2] < layer_budget:
        ...
        if not should_compress or key_states.shape[-2] < layer_budget:
            return key_states, value_states
    ...
    def _pack_layer(self, entry, attn_cache):
        ...
        keep_count = min(
            self._layer_historical_budget(entry["layer_idx"]),
            attn_cache.shape[-1],
        )
        indices = attn_cache.topk(keep_count, dim=-1).indices
        layer_budget = keep_count + self.window_size

        if self.record_kept_token_indices:
            self._record_kept_indices(indices, attn_cache, kv_cache_len, layer_budget)
    ...
    def _layer_cache_budget(self):
        return self._layer_historical_budget(self.layer_idx) + self.window_size

    def _layer_historical_budget(self, layer_idx):
        per_layer_history = self.budget - self.window_size
        if self.hidden_mix_profile is None or not self.hidden_mix_profile.get("use_layer_budget_scores"):
            return per_layer_history
        return self._layer_historical_budgets().get(int(layer_idx), per_layer_history)

    def _layer_historical_budgets(self):
        per_layer_history = self.budget - self.window_size
        profile_layers = tuple(int(layer) for layer in self.hidden_mix_profile.get("profiled_layers", ()))
        layer_scores = self.hidden_mix_profile.get("layer_budget_scores", {})
        total_score = float(self.hidden_mix_profile.get("layer_budget_score_total", 0.0))
        layer_count = len(profile_layers)
        if layer_count < 1 or total_score <= 0.0:
            return {}

        total_hist_budget = layer_count * per_layer_history
        if total_hist_budget <= layer_count:
            return {layer: 1 for layer in profile_layers}

        remaining_budget = total_hist_budget - layer_count
        budgets = {}
        remainders = []
        assigned = layer_count
        for offset, layer in enumerate(profile_layers):
            raw_budget = remaining_budget * float(layer_scores[layer]) / total_score
            extra_budget = int(math.floor(raw_budget))
            budgets[layer] = 1 + extra_budget
            assigned += extra_budget
            remainders.append((raw_budget - extra_budget, -offset, layer))

        for _fraction, _neg_offset, layer in sorted(remainders, reverse=True)[: total_hist_budget - assigned]:
            budgets[layer] += 1
        return budgets

    def _record_kept_indices(self, indices, attn_cache, kv_cache_len, layer_budget):
        ...
        self.evicted_token_num += kv_cache_len - layer_budget
