#!/bin/bash
# Needle-in-a-Haystack on Qwen3-4B
# Tests at budget=128L and budget=1024L

MODEL_PATH="Qwen/Qwen3-4B-Instruct-2507"  # e.g., meta-llama/Llama-3.1-8B-Instruct
...
cleanup() {
    ...
    if ((${#PIDS[@]} > 0)); then
        echo
        echo "Interrupted. Stopping running Needle Qwen3-4B jobs..."
...
for exp in "${EXPERIMENTS[@]}"; do
    ...
    compression_args=()
    version_args=(--model_version "Qwen3-4B_FullKV_${capacity}")
...
wait_for_batch
echo "All Needle Qwen3-4B experiments completed."
